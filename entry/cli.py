...
app = typer.Typer(help="Nodeclaw - 极客专属的赛博智能终端")
console = Console()

node_style = questionary.Style([
...
def config_wizard():
    console.clear()
    console.print(Panel(
        "👾 Welcome to [bold #8d52ff]Nodeclaw[/bold #8d52ff]...\n\n☁️[dim] 请完成模型配置，我们将把密钥安全固化在本地。[/dim]", 
        title="[bold white]✦  Nodeclaw Config[/bold white]", 
        border_style="#8d52ff"
    ))
    ...
    if not provider_raw:
        console.print("[dim #8d52ff]✦   录入中断，Nodeclaw 配置已取消。[/dim #8d52ff]")
        return
    ...
    console.print(Panel(
        ...
        f"👉 输入 [bold #00ffff]nodeclaw run[/bold #00ffff] 即可启动系统！",
        border_style="#00ffff"
    ))

def _show_boot_error():
    console.print(Panel(
        "[bold #00ffff]Nodeclaw未完成配置![/bold #00ffff]\n\n"
        "[#8d52ff]检测到 API Key、模型或Baseurl。请重新执行以下命令完成配置：[/#8d52ff]\n"
        "[bold #00ffff]nodeclaw config[/bold #00ffff]",
...
def run_agent():
    ...
    import entry.main as nodeclaw_main
    nodeclaw_main.main()
...
def run_monitor():    
    ...
    try:
        import entry.monitor as nodeclaw_monitor
        nodeclaw_monitor.main()
