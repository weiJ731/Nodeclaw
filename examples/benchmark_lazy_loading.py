...
from nodeclaw.core.skill_loader import load_dynamic_skills, get_skill_count, clear_skill_cache
...
def benchmark():
    """性能基准测试"""
    print("=" * 70)
    print("Nodeclaw 懒加载性能基准测试")
    print("=" * 70)
    
    # 创建临时测试目录
    temp_dir = tempfile.mkdtemp(prefix="nodeclaw_bench_")
    
    # 修改环境变量
    original_env = os.environ.get("NODECLAW_WORKSPACE")
    os.environ["NODECLAW_WORKSPACE"] = temp_dir
    ...
    try:
        ...
        for num_skills in test_sizes:
            ...
            # 重新加载配置
            import importlib
            import nodeclaw.core.config as config_module
            import nodeclaw.core.skill_loader as skill_loader_module
        ...
        if original_env is not None:
            os.environ["NODECLAW_WORKSPACE"] = original_env
        else:
            os.environ.pop("NODECLAW_WORKSPACE", None)
