...
def get_embedding_model(model_name: str | None = None):
    """
    获取 Embedding 模型 (用于 V2 记忆系统的向量化)
    🔥 修复：使用原生的 DashScopeEmbeddings 绕过 OpenAI 兼容层的格式 Bug
    """
    api_key = os.getenv("OPENAI_API_KEY")  # .env 里配置的其实是百炼 Key
    
    if not api_key:
        raise ValueError("未找到 API Key！请检查 .env 配置文件。")

    # 直接调用阿里云原生接口，格式绝对标准
    return DashScopeEmbeddings(
        model=model_name or os.getenv("EMBEDDING_MODEL", "text-embedding-v2"),
